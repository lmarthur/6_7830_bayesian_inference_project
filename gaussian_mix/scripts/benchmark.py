...
a unified comparison table and plots. Metrics are chosen to be algorithm-agnostic:
  ...
  - wall_time_s          : total wall-clock time (universal cost axis)
  ...
  - ess_per_wall_s       : ess_bulk_arviz / wall_time_s  (primary efficiency metric)
...
def print_table(results: list[dict]) -> None:
    header = (
        f"{'Algorithm':<10}  {'Wall(s)':>8}  {'ESS':>8}  {'ESS/s':>8}  "
    ...
    for r in results:
        ...
        print(
            f"  {r['algorithm']:<10}  {r['wall_time_s']:>8.1f}  "
            f"{r['ess_bulk_arviz']:>8.0f}  {r['ess_per_wall_s']:>8.2f}  "
...
def make_plots(results: list[dict]) -> None:
    ...
    bars = ax.bar(x, [r["wall_time_s"] for r in good], width=bar_w, color="steelblue", alpha=0.85)
    ...
    # ---- Figure 3: ESS per wall-second ----
    fig, ax = plt.subplots(figsize=(8, 4))
    bars = ax.bar(x, [r["ess_per_wall_s"] for r in good], width=bar_w, color="seagreen", alpha=0.85)
    ax.bar_label(bars, fmt="%.2f", padding=3, fontsize=8)
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_ylabel("ArviZ bulk ESS / wall-second")
    ax.set_title("Sampling efficiency (higher is better)")
    ax.yaxis.set_minor_locator(ticker.AutoMinorLocator())
    fig.tight_layout()
    fig.savefig(BENCHMARK_OUTPUT_DIR / "ess_per_wall_second.png", dpi=150, bbox_inches="tight")
    plt.close(fig)

    # ---- Figure 4: Pareto scatter — wall time vs. mode MAE ----
    fig, ax = plt.subplots(figsize=(7, 5))
    for r in good:
        ax.scatter(r["wall_time_s"], r["mode_weight_mae"], s=80, zorder=3)
        ax.annotate(
            r["algorithm"].upper(),
            (r["wall_time_s"], r["mode_weight_mae"]),
